...
public:
	...
	bool depthFirstSearch(int startIndex, int endIndex)
	{
		...
		std::cout << vertices[startIndex].getNode() << endl;

		//Declare a stack for searching
		stack<int> stackSearch;
		...
		while (!stackSearch.empty())
		{
			...
			if (vert == -1)
			{
				...
				std::cout << vertices[vert].getNode() << endl;

				//Push vertex index into stack
				stackSearch.push(vert);
			}
	...
	bool depthFirstSearch(char startVertex, char endVertex)
	{
		...
		//Print the value at the start index
		cout << vertices[startIndex].getNode() << endl;
		...
		while (!stackSearch.empty())
		{
			...
			} else if (vert != -1) { //If vertex is not -1
				//Set visited vertex to 1
				vertexVisits[vert] = 1;
